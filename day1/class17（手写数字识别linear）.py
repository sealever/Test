...
class Network(nn.Module):
    def __init__(self, in_features, num_class, units=None):
        ...
        layers = []
        for unit in units:
            layers.append(nn.Linear(in_features=in_features, out_features=unit))
            layers.append(nn.ReLU())
            in_features = unit
        layers.append(nn.Linear(in_features=in_features, out_features=self.num_class))
    ...
    def forward(self, x):
        ...
        """
        x = x.reshape(-1, self.in_features)  # [N,1,28,28] -> [N,784]
...
def training():
    ...
    # 2.定义模型
    net = Network(in_features=1 * 28 * 28, num_class=10)
...
if __name__ == '__main__':
    training()
    # export(model_dir='../output/05/230830/model')
    # load_model(model_dir='../output/05/230830/model')
